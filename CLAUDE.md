...
**Fix glue-mc-interchange — motor de reglas asignaba fees incorrectos por monotonically_increasing_id() inestable (2026-06-26):** `assign_rules_simple()` usaba `monotonically_increasing_id()` como llave de join entre transacciones (`work`) y reglas rankeadas (`ranked_rules`). `monotonically_increasing_id()` no es estable entre shuffles Spark — tras el cross-join con las reglas y el `Window.partitionBy(work_id)`, los IDs del `work` original no coincidian con los del `ranked_rules`, asignando reglas a transacciones incorrectas. Sintoma: conteos y montos correctos, fees incorrectos en todos los MTIs 1240/1442. Solucion (equipo): reemplazar `work_id` por `Window.partitionBy(file_id, file_idn, ref_id)` + join por esas 3 llaves estables — mismo patron ya validado en `lmbd-mc-store` para el merge CLN+CAL+ITX. Validado con reprocesso SBSA MC enero 2026 (104/104 SUCCEEDED). Detalle en `.claude/memory/mc_interchange_motor_fix.md`.

**Fix glue-vi-interchange + get_transaction.py — product_id CLN sobreescribia el valor ARDEF-derivado del CAL (2026-06-28):** `process_output()` hacia join CLN+CAL sin resolver conflictos de nombres — `product_id` existe en ambos (CLN: raw `' '`; CAL: ARDEF-derivado `'F'/'G'/etc.'`). Pandas usaba el CLN por posicion → `product_id=' '` → normalizado a `"BLANK"` → motor asignaba catch-alls → fees SBSA -50% ZAR. Fix: antes del join, columnas de CLN que tambien estan en CAL se renombran con sufijo `_cln` (`product_id → product_id_cln`); CAL queda como autoritativo. Simultaneamente: `interchange_cols` eliminaba `source_amount`/`source_currency_code_alphabetic` (eran pass-through del CLN, generaban columnas `_itx` duplicadas en operational); `get_transaction.py` corregido para leer `interchange_fee_amount_itx` (fee calculado) en vez de `interchange_fee_amount` (raw CLN = 0). Validado: reprocesso SBSA VI enero 2026 (156/156 interchange + 156/156 store). Detalle en `.claude/memory/vi_interchange_product_id_and_fee_naming_fix.md`.

**Fix lmbd-mc-store — OOM en bloques CLN grandes por carga completa en pandas (2026-06-23):** `_store_output()` cargaba el Parquet CLN completo en memoria (`pq.read_table().to_pandas()`). Para archivos grandes como SBSA MC IN 2026-01-03 (1.86 GB, bloque mayor 298 MB CLN → ~10+ GB en pandas por columnas string), el Lambda moria con `Runtime.OutOfMemory`. Solucion: streaming con `pq.ParquetFile.iter_batches(batch_size=ITX_STORE_BATCH_SIZE, default=100k)` — CLN se descarga completo (bytes) pero se itera por batches; CAL e ITX se cargan completos en memoria (son pequenos: ~14 MB y ~27 MB). Schema de salida derivado del primer batch y forzado en los siguientes via `_restore_schema(table, cln_dtype_map, output_schema)`. Escritura incremental a `pq.ParquetWriter` sobre `BytesIO`, subida unica a S3 al terminar. Pico de memoria bajo de ~10 GB a 2,946 MB (validado en SBSA: 33 bloques, 2,445,146 records, 353s). Config actualizada: Timeout 300s → 900s. Variable de entorno opcional: `ITX_STORE_BATCH_SIZE`. Detalle en `.claude/memory/gotchas.md`.
