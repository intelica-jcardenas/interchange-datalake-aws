# Gotchas archivados (detalle completo)

Este archivo contiene el detalle completo (síntoma, causa raíz, metodología de debugging, validación) de gotchas que fueron resumidos en `gotchas.md` para reducir el contexto cargado automáticamente en cada conversación (este archivo **no** está referenciado vía `@` en CLAUDE.md).

Cada entrada aquí tiene su versión resumida correspondiente en `gotchas.md` con un pointer hacia esta. Si necesitas el "por qué" completo de un fix, búscalo aquí.
...
**Estado:** Resuelto para `EBGR/VISA/baseii_drafts/file_type=IN` (56/56 archivos). Validado por el re-run de `glue-test-1` documentado en el gotcha anterior (561,711 filas leídas sin error de schema).
...
**Estado:** Resuelto en código local (2026-06-09). El fix está incluido en el `calculate.py` usado por el reproceso masivo de la sesión 2026-06-11 (ver `manual_execution.md`), pero no se validó específicamente con un comparativo de `timeliness` contra legacy tras ese reproceso.
...
**Estado:** Resuelto y validado (re-ejecutado `glue-vi-calculate` 2026-06-06, `debug_ardef_join.py` confirmó match ~100%). Reprocesado masivamente para EBGR enero 2026 en la sesión 2026-06-11 (ver `manual_execution.md`).
...
**Estado:** Resuelto en código local (2026-06-10). Pendiente subir el handler actualizado al Lambda `lmbd-mc-interpreter` y validar end-to-end con `itl-0004-itx-dev-intchg-02-sfn-mc`.
...
**Nota:** Este mismo patrón debe verificarse en `glue-vi-interchange` si alguna vez se presenta el mismo síntoma. (Aplicado también en `lmbd-mc-transform` — ver `decisions.md`).
...
**Estado:** Resuelto. Esta lógica fue posteriormente generalizada a `_cal_dtype_map` (ver gotcha de NullType arriba).
