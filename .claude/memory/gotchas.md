...
## lmbd-router / lmbd-unzip: archivo sin match de patrón en `file_pattern` (DynamoDB) se descartaba en silencio, sin rastro persistente — RESUELTO, DESPLEGADO Y VALIDADO CONTRA AWS REAL 2026-08-12

**Archivos:** `lambdas/router/src/handler.py`, `lambdas/unzip/src/handler.py`. Síntoma original (confirmado 2026-08-11, ver detalle histórico abajo): un archivo sin match en `clasificar_archivo()` (router) se descartaba con solo un `logger.warning` — sin registro en `file_control`, sin moverse de `s3-landing`, sin ninguna traza real. Extendido el mismo día a `lmbd-unzip`, que tenía el MISMO problema de forma independiente para archivos entregados dentro de un ZIP (ver más abajo).

**Fix (2026-08-12) — 2 funciones nuevas, reusan toda la lógica de identidad ya existente:**
- `procesar_archivo_desconocido()` — calcula `file_id`/`content_hash` igual que un archivo clasificado (`generar_file_id`/`calcular_content_hash`), registra en `file_control` con `control_status='UNKNOWN'` + `error_message='Sin match de patrón en file_pattern'`, y archiva.
- `procesar_archivo_duplicado()` — mismo tratamiento para CUALQUIER duplicado detectado (clasificado o no, unificando el caso que antes tampoco archivaba nada). Deliberadamente NO usa `registrar_archivo()` (que hace `put_item`, reemplazaría el registro completo con la historia real de la corrida original) — usa `update_item` con `list_append` para agregar un evento a una lista `duplicate_uploads`, sin tocar ningún otro campo del registro existente.
- Ambas archivan a `s3-archive/{client_id}/originals/{UNKNOWN|DUPLICATE}/{año}/{mes}/{archivo}.zip` — comprimido en streaming (`_compress_and_save_to_tmp`/`_upload_zip_to_s3`, port literal del mismo mecanismo de `lmbd-archive-file`, simple/multipart según tamaño), con verificación (`head_object`) antes de borrar el original de landing.
- `EphemeralStorage` del router subido de 512MB a 2048MB (necesario para comprimir a `/tmp`); nueva env var `S3_BUCKET_ARCHIVE` agregada (antes el router no la tenía — nunca escribía a `s3-archive` directamente).

**Validado en vivo contra AWS real (no solo `py_compile`):** archivo sintético subido 2 veces a `s3-landing/EBGR/` con un nombre que no matchea ningún patrón real de EBGR.
- 1ra subida → `file_control` con `control_status=UNKNOWN`, `brand_id=UNKNOWN`, `file_type=UNKNOWN`, `error_message` correcto; archivado en `EBGR/originals/UNKNOWN/2026/08/....zip`; landing limpio.
- 2da subida (mismo contenido) → detectado como duplicado; **el registro original no se pisó** (`control_status` siguió `UNKNOWN`); se agregó a `duplicate_uploads` (`detected_at`/`landing_path`/`archived_key`); archivado en `.../DUPLICATE/...zip`; landing limpio de nuevo.
- Artefactos de prueba borrados después de validar (S3 + el item de DynamoDB sintético).
- **Duplicado de un archivo YA CLASIFICADO — validado 2026-08-12 (sesión aparte, sin riesgo de disparar un Step Function real):** se sembró manualmente en DynamoDB un registro simulando un archivo ya procesado con éxito (`control_status='SUCCESS'`, `file_id`/`content_hash` calculados de antemano para un archivo de prueba con nombre que matchea el patrón real de EBGR `(?:[Ii]\d{4})`), y se subió el mismo contenido a landing — la detección de duplicado ocurre *antes* de `start_process()` en `lambda_handler()`, así que nunca se llegó a disparar ningún Step Function. Resultado: `control_status` siguió `SUCCESS` intacto, `duplicate_uploads` agregado, archivado a `DUPLICATE/`, landing limpio — mismo comportamiento que el caso sin clasificar. Artefactos de prueba borrados tras validar.

**Detalle histórico (síntoma original, 2026-08-11):** `clasificar_archivo()` recorre los patrones activos de DynamoDB `file_pattern` en orden de prioridad (`re.search`), devuelve `None` si ninguno matchea — antes de este fix, eso terminaba en `results.append({'status': 'SKIPPED', 'reason': 'No pattern match'})` sin ningún efecto en S3/DynamoDB. Detectado investigando qué pasaría con un archivo de reglas Visa (u otro tipo nuevo) con un nombre que no matchea ningún patrón configurado (typo en el regex, tipo de archivo nuevo del cliente, etc.).

**`lmbd-unzip` tenía el MISMO problema, de forma independiente — encontrado con la "prueba oficial" del usuario (ZIP real de NXGR, 2026-08-12):** el fix de arriba solo cubre archivos que llegan directo a `s3-landing`. `lmbd-unzip` tiene su propia lógica de matching (`_load_patterns()`/`_matches_pattern()`), separada y ANTERIOR a la del router — descartaba en silencio cualquier archivo interno de un ZIP sin match, antes de que llegara a landing, así que el fix del router no tenía ningún efecto sobre archivos entregados dentro de un ZIP. Confirmado en CloudWatch con el ZIP real `NEXI-INTELICA-MC-IN-20250707_Jul-06.zip` (6 archivos, cliente NXGR, ninguno con match): `"Loaded 2 patterns for 'NXGR'"` → 6× `"SKIP: ... (no pattern match)"` → `"Summary: 0 uploaded, 6 skipped"` — el ZIP en sí se archivaba normalmente, pero los 6 archivos internos no dejaban ningún rastro (ni en landing, ni en `file_control`, ni en `s3-archive`). **Fix:** `lmbd-unzip` ahora sube TODOS los archivos extraídos a landing sin filtrar por match (`pattern_id=None` para los sin match) — cada uno re-dispara el router vía S3 event, que aplica el mismo tratamiento UNKNOWN/DUPLICATE ya validado arriba.

**Caso relacionado — cliente con CERO patrones activos configurados (no "sin match", sino "sin ningún patrón"), 2026-08-12:** rompía `lmbd-unzip` y `lmbd-router` de formas distintas. En `lmbd-unzip`, lanzaba `ValueError` y abortaba el ZIP COMPLETO (nunca se archivaba, landing nunca se limpiaba, ningún archivo individual se procesaba). En `lmbd-router`, tomaba una rama de early-exit separada (`if not patrones: ... status: ERROR`) que nunca llegaba a llamar `procesar_archivo_desconocido()`. Fix en ambos: eliminado el hard-fail/early-exit — `_matches_pattern()`/`clasificar_archivo()` ya devuelven `None` de forma natural con una lista de patrones vacía, así que ese caso fluye solo por el mismo camino que "sin match", sin lógica nueva.

**`archive_key` no se persistía en DynamoDB para el registro `UNKNOWN` (mismo día):** `procesar_archivo_desconocido()` calculaba la ruta de archivado y la devolvía en la respuesta del Lambda, pero nunca la guardaba — a diferencia de `procesar_archivo_duplicado()`, que sí la persiste (dentro de `duplicate_uploads`). Fix: `update_item` no destructivo (`SET archive_key = :ak`) justo después de archivar con éxito. **Formato corregido en una segunda pasada** — el usuario señaló que el flujo normal (Step Functions, ver `decisions.md` para el detalle) guarda `archive_key` como un string cuyo VALOR es el JSON `{"status":"ARCHIVED","file_id":"...","archive_key":"..."}`, no la ruta pelada; corregido para igualar ese formato (`json.dumps(...)`), incluyendo una corrección retroactiva de los registros de prueba ya escritos con el formato viejo.

**Validación end-to-end real en producción, cierre del hallazgo de `lmbd-unzip` (2026-08-12):** el usuario volvió a subir el mismo tipo de ZIP real de NXGR (`NEXI-INTELICA-MC-IN-20250708_Jul-07.zip`, 6 archivos sin match) tras el deploy — 6 registros `UNKNOWN` reales en `file_control` con `archive_key` en el formato correcto, `s3-landing/NXGR/` limpio, los 6 `.zip` presentes en `s3-archive/NXGR/originals/UNKNOWN/2026/08/`. Confirma que el fix de `lmbd-unzip` (que antes descartaba estos mismos archivos en silencio) funciona end-to-end. Registros/archivos de esta prueba de validación borrados después de confirmar — a diferencia de la prueba oficial original (`_Jul-06.zip`), que el usuario pidió preservar sin tocar y sigue intacta en `s3-archive/NXGR/originals/zip/2025/07/`.

**Si vuelve a aparecer un caso similar** (archivo descartado en silencio dentro de un flujo con más de un punto de entrada a landing): sospechar de lógica de matching/filtrado duplicada e independiente entre Lambdas que comparten el mismo propósito (router vs. unzip) — no asumir que arreglar uno cubre al otro.
...
**Validación local (pandas, 2026-07-27, sin Spark/AWS):** replicado el diseño completo (dedup único + resolución en el join) contra los datos crudos reales de ambos grupos:
...
**Detectado:** 2026-07-09, durante revisión de `get_transaction.py` (comparativo agregado SBSA enero 2026).
...
**Si vuelve a aparecer:** escanear el footer/schema de los Parquets afectados (`pq.ParquetFile(path).schema_arrow` vía boto3+pyarrow, sin descargar el archivo completo — script de ejemplo borrado en la limpieza de `tst_files/` del 2026-08-13, recrear si hace falta) para listar archivos/columnas con NullType, mapear via `file_control` (scan por rango de fechas + `control_status=PARTIAL_SUCCESS`) y reprocesar con `lmbd-vi-store`.
