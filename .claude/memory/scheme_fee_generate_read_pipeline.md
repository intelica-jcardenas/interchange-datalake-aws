...
**Desambiguación del duplicado on-us (2026-07-10, ajustado a literales exactos del legacy tras revisar `getquery.py`):** `file_id`+`row_id`/`app_id` por sí solos son idénticos entre la fila original y su copia on-us (`_apply_duplicate_on_us()` solo cambia `business_mode_code`/`business_mode_id`). La llave completa para el join es `file_id` + `row_id` + `business_mode_code` (traducido A/I ↔ ACQUIRING/ISSUING en el momento del join). En `scheme_fee.py`, `table_description` ahora replica exacto los literales de `get_on_us_visa`/`get_sms_on_us_visa`/`get_on_us_mastercard` (legacy): duplicado VISA BASEII = `"VISA ON-US DUP (ACQ TO ISS)"`, SMS = `"VISA ON-US DUP (SMS TO ISS)"`, MC = `"MASTERCARD ON-US DUP (ACQ TO ISS)"` — literales fijos pasados como parámetro a `_apply_duplicate_on_us()`, no derivados del `table_description` original. De paso, las filas normales de VISA BASEII también pasaron a distinguir `"VISA ACQ"`/`"VISA ISS"` (antes un único `"VISA"`) y MasterCard a `"MASTERCARD ISS AND ACQ"` (antes `"MasterCard"`), replicando exacto lo que hace legacy en `get_acquirer_visa`/`get_issuers_visa`/`get_transactions_mastercard`. Sirve como marcador explícito adicional al join — no depende únicamente de `business_mode_id`, campo con historial de inconsistencia de mayúscula/minúscula entre Visa y Mastercard (ver `gotchas.md`). `get_transaction.py` no lleva columna espejo; el join sigue traduciendo `business_mode_code` de ese lado.
